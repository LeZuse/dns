module.exports = function () {
    ...
    Dns.prototype.resolver = function(req, res, wildcard_hostname) {
        var $this        = this,
            question     = res.question[0],
            hostname     = question.name,
            length       = hostname.length,
            answer       = {},
            key          = this.config.prefix + (wildcard_hostname || hostname);
        ...
        this.store.get(key, function (err, value) {
            ...
            if (!value && !wildcard_hostname) {
                var chunks = hostname.split('.');
                if (chunks.length > 1) {
                    chunks[0] = '*';
                    var wildcard_hostname = chunks.join('.');
                    return $this.resolver(req, res, wildcard_hostname);
                }
            }
            var processRecord = function(object) {
            ...
            if (!err && value && (value instanceof Array && value.length > 0) || typeof value === 'string') {
